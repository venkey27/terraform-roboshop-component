...
resource "aws_lb_target_group" "main" {  # target consist of instances 
  ...
  port     = var.component == "frontend" ? "80" : "8080"   # if component is frontend then port 80, if components are backend then port 8080
  protocol = "HTTP"
  vpc_id   = local.vpc_id
...
resource "aws_lb_listener_rule" "main" { # you are telling the Application Load Balancer how to route traffic to your different 
  listener_arn = local.alb_listener_arn        # if component is frontend, fronend_alb_listener will comes or else banckend_alb_listener comes here
  priority     = var.rule_priority

  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.main.arn   # Purpose: THEN do this. Because the condition matched, the Load Balancer says: 
